...
const BLOG_POSTS = [
  {
    slug: "bank-loan-portfolio-risk",
    title: "The 14% of loans quietly dragging the book down",
    dek: "A personal project modeling 38.6K loan applications turned into a lesson on how a single bad-loan segment can hide inside a portfolio that looks healthy in aggregate.",
    date: "2026-07-10",
    readTime: "4 min read",
    tags: ["Data Analyst", "Power BI", "Projects"],
    content: [
      "I wanted a project that forced me to think about risk, not just reporting, so I picked a loan portfolio dataset: 38.6K applications and $435.8M in funded amount, with no single view tying application volume, funding, and loan quality together.",
      "First step was just consolidation, getting application and funding data into one SQL-backed model so I could ask questions across the whole book instead of one report at a time. Once that was in place, the real work was in the Power BI layer: views for application volume, total funded amount, and a breakdown of loan status.",
      "The interesting part was isolating the bad-loan segment. It came out to about 14.1% of the book, small enough to hide inside a healthy-looking blended repayment rate, large enough that anyone underwriting the next batch of loans should care about it.",
      "I modeled how interest rate and debt-to-income trends related to that segment, mostly to see whether the risk was concentrated in identifiable conditions or spread evenly across the book. It wasn't evenly spread. Certain rate and DTI combinations showed up disproportionately inside the bad-loan slice.",
      "Nobody assigned me this project, which is exactly why I picked it. Growth analytics at Practo taught me to distrust blended numbers. This was a chance to apply that same instinct somewhere I had to build the whole pipeline myself, dataset to dashboard, with nobody downstream to catch what I missed.",
    ],
  },
  {
    slug: "learning-power-bi-goa",
    title: "Learning Power BI on a laptop in Goa",
    dek: "Before Practo, before the MSc, I was a remote MIS Executive teaching myself Power BI from scratch to fix a reporting process that was eating half a day at a time.",
    date: "2026-05-30",
    readTime: "4 min read",
    tags: ["Career", "Power BI", "Early Career"],
    content: [
      "My first real analytics job wasn't glamorous. I was a remote MIS Executive for a small arts and handicrafts business in Goa, and the entire sales reporting process was manual: someone pulled numbers by hand across 500+ SKUs and 2 to 3 outlets, a job that ate roughly half a day every time a report was due.",
      "Nobody handed me Power BI training. I taught it to myself, mostly by building the thing I actually needed: a live dashboard that could answer \"how did we do this week, by product, by category, by outlet\" without anyone opening five spreadsheets.",
      "The data itself needed work before any of that was possible. I wrote SQL queries and leaned on Excel and Google Sheets to pull and clean sales and stock data across all 500+ products, since the source data was never clean enough to plug straight into a dashboard.",
      "Once the reporting was live, I got curious about a smaller question: whether one type of promotional offer actually outperformed another, instead of just assuming the more generous-looking one won. I ran an actual A/B test on two offers. The premium offer won on sales, and that result shaped how later promotions got structured.",
      "It's a small story next to a 4M-user healthtech platform, but it's the one that taught me the actual habit: don't trust a manual process just because it's always been manual, and don't trust an assumption about what's working until you've tested it. Everything after this job has mostly been that same instinct applied to bigger data.",
    ],
  },
  {
    slug: "teaching-a-bot-to-say-yes",
    title: "Teaching a bot to say yes",
    dek: "Building an AI PR reviewer for Practo's engineering org meant deciding, carefully, when a machine gets to approve someone's code and when it has to hand the decision back to a human.",
    date: "2026-02-25",
    readTime: "5 min read",
    tags: ["AI Engineering", "LangGraph", "Practo"],
    content: [
      "Every pull request needed a human to catch the same handful of recurring issues: unhandled errors, string-built SQL, a function that had grown too large to reason about. That first pass ate reviewer time on every single PR, on every team, regardless of how experienced the reviewer was.",
      "I built four LangGraph agents that run in parallel the moment a PR opens: one for static analysis, one for security issues, one for architecture, one for style. Running them in parallel instead of one long sequential pass meant results landed in seconds instead of minutes.",
      "The harder design question wasn't the analysis, it was approval. I didn't want a bot rubber-stamping code just because static checks passed. So the approval logic works the other way around: a human defines the checklist for what \"good\" means for their repo, and only once a PR satisfies every item on that specific checklist does it get auto-approved. Even then, the actual logic, the part a checklist can't verify, still needs a human to check the script or validate it works as intended. The bot handles the repetitive part. The judgment call stays a judgment call.",
      "I tested the whole thing locally first with Docker Compose and fake GitHub/LLM clients before touching anything real, then deployed it on Practo's existing Bacron/Prefect workflow infrastructure instead of standing up a dedicated server. No team had to provision or maintain anything new to use it.",
      "It's been running long enough now that the thing I notice most isn't the time saved, it's how much less friction there is around raising the same feedback for the tenth time on a different PR. That used to be the tedious part of reviewing code. Now it isn't anyone's job.",
    ],
  },
  {
    slug: "sem-fraud-twelve-lakh",
    title: "Twelve lakh a month, and nobody had noticed",
    dek: "Blended SEM performance looked fine. Splitting it by channel showed three campaigns quietly burning real budget on installs that were never going to convert.",
    date: "2026-01-19",
    readTime: "4 min read",
    tags: ["Analytics", "SEM", "Practo"],
    content: [
      "Paid marketing spend gets reviewed constantly, but usually at the campaign level: cost per install, blended CAC, month-over-month trend. Those numbers looked stable. Nobody was flagging anything.",
      "I wanted to check something more specific: whether the ratio of clicks to actual installs held steady across channels, or whether some channels were paying for clicks that never converted into real users. I pulled click and install data for three SEM campaigns and compared the ratios side by side instead of looking at each one in isolation.",
      "Two of them looked normal. One didn't. Its click-to-install ratio was far out of line with the other two, the kind of gap that usually means bot traffic or click farms rather than real interest.",
      "I flagged it, and once we looked closer it turned out all three campaigns sourced from the same vendor had the same problem, roughly ₹12L a month in spend that was never buying real users. We paused all three.",
      "What stuck with me is how invisible this kind of thing is until you go looking for it on purpose. Nothing about the top-line numbers said \"problem here.\" You only catch it by comparing what should have been comparable and noticing when it isn't.",
    ],
  },
  {
    slug: "chasing-the-34-percent-drop",
    title: "Chasing a 34% drop across half a million sessions",
    dek: "A payment completion rate that fell off a cliff turned out to be a two-line bug that only fired for one kind of user.",
    date: "2025-12-08",
    readTime: "4 min read",
    tags: ["Analytics", "Root Cause Analysis", "Practo"],
    content: [
      "Payments dropped 34% almost overnight and nobody could say why. The obvious suspects, a pricing change, a broken checkout button, a competitor promotion, all checked out clean. Whatever broke wasn't visible from the top.",
      "I pulled 500K+ payment sessions and started splitting them by everything I could think of: device type, OS version, payment method, time of day. Most cuts looked identical to the baseline. Then I split by network quality alongside device, and one slice stood out immediately: low-connectivity users on a specific device tier were failing at a much higher rate than everyone else.",
      "That narrowed it fast. I handed the segment to Product with the exact conditions under which it failed, and they traced it to an SDK bug that only surfaced under poor network conditions, a case the original testing hadn't covered because most testing happens on strong office wifi.",
      "Product shipped a fix within two weeks and the conversion came back.",
      "The lesson wasn't really about SQL or segmentation technique. It was that \"34% drop\" is a symptom, not a diagnosis, and the fix only shows up once you're willing to keep splitting the data past the point where it stops being convenient.",
    ],
  },
  {
